...
class VICatSR(Algorithm):
    ...
    def train_simple_case(self, data):

        theta = torch.tensor([0.7], requires_grad=True)

        def p_x(theta, x):
            return torch.stack([theta, 1 - theta])[x]

        for i in range(50):

            q = torch.distributions.categorical.Categorical(probs=torch.tensor([theta, 1 - theta]))

            sampled_z = []
            for i in range(100):
                sampled_z.append(q.sample())

            likelihoods = []
            for z in sampled_z:
                likelihoods.append(0.9 if z == 1 else 0.3)

            loss = statistics.mean(likelihoods)

            loss_grads = []
            for l, z in zip(likelihoods, sampled_z):

                prob = p_x(theta, z)
                prob.backward()

                loss_grads.append((l * theta.grad).item())

                theta.grad.zero_()

            loss_grad = statistics.mean(loss_grads)

            print('Loss:', loss)
            print('Loss grad:', loss_grad)
            print('Theta:', theta)

            new_theta = theta + 0.1 * loss_grad
            new_theta = torch.clamp(new_theta, min=0.0, max=1.0)

            theta = torch.tensor(new_theta, requires_grad=True)
